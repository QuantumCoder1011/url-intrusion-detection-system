...
Uses the same backend logic (DataIngestion + DetectionEngine) as the running app.
"""
import sys
...
def process_file(filepath: Path, file_type: str, engine: DetectionEngine, ingestion: DataIngestion):
    """Process one file and return (urls_list, detections_list)."""
    urls = ingestion.process_file(str(filepath), file_type)
    detections = []
    for u in urls:
        url = u.get("url", "")
        for attack in engine.analyze_url(url):
            detections.append({
...
def main():
    ...
    if not csv_path.exists():
        ...
        return
    urls_csv, det_csv = process_file(csv_path, "csv", engine, ingestion)
    ...
    if not pcap_path.exists():
        ...
        urls_pcap, det_pcap = process_file(pcap_path, "pcap", engine, ingestion)
